...
use std::{collections::HashSet, fmt::{Debug, Display}, sync::Arc};
...
type MultiThreadedContextDelta<CCI> = ContextDelta<CCI, SharedDirectConceptDelta<CCI>>;

#[derive(Clone)]
pub struct SharedContextDelta<CCI: Clone + Display>(pub Arc<MultiThreadedContextDelta<CCI>>);
...
impl<'a, CCI: Clone + Display> From<&'a mut SharedContextDelta<CCI>> for Option<&'a mut MultiThreadedContextDelta<CCI>> {
    fn from(scd: &'a mut SharedContextDelta<CCI>) -> Self {
        Arc::get_mut(&mut scd.0)
    }
}

impl<CCI: Clone + Display> From<MultiThreadedContextDelta<CCI>> for SharedContextDelta<CCI> {
    fn from(mtcd: MultiThreadedContextDelta<CCI>) -> Self {
        Self(mtcd.into())
    }
}

impl<CCI: Clone + Debug + Display + Eq + Hash> From<SharedContextDelta<CCI>> for MultiThreadedContextDelta<CCI> {
    fn from(scd: SharedContextDelta<CCI>) -> Self {
        Arc::try_unwrap(scd.0).unwrap()
    }
}

impl<CCI: Clone + Display> From<SharedContextDelta<CCI>> for Option<MultiThreadedContextDelta<CCI>> {
    fn from(scd: SharedContextDelta<CCI>) -> Self {
        Arc::try_unwrap(scd.0).ok()
    }
}

impl<CCI: Clone + Display> AsRef<MultiThreadedContextDelta<CCI>> for SharedContextDelta<CCI> {
...
pub type SharedDirectConceptDelta<CCI> =
    Arc<DirectConceptDelta<CCI>>;


impl<'s, 'v, S, CCI: Clone + Display + Copy + Debug + Eq + Hash + Send + Sync + From<usize>> ContextSearchIteration
    for ContextSearch<
        's,
        'v,
        S,
    ...
    >
where
    S: SnapShotReader<SharedDirectConceptDelta<CCI>, ConceptId = CCI> + Sync + Debug,
    for<'a> &'a std::collections::HashSet<CCI>: rayon::iter::IntoParallelIterator<Item=&'a CCI>,
{
