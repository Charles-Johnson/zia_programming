...
impl Default for InferenceRuleSnapShot {
    fn default() -> Self {
        ...
        let mut cause_concept = Concept::composition_of(4, &mut concept_a, &mut variable_concept);
        let mut result_concept = Concept::composition_of(2, &mut variable_concept, &mut concept_b);
        let mut implies_result_concept = Concept::composition_of(5, &mut implication_concept, &mut result_concept);
        let mut cause_implies_result_concept = Concept::composition_of(6, &mut cause_concept, &mut implies_result_concept);
        cause_implies_result_concept.make_reduce_to(&mut true_concept);
        let mut example_composition = Concept::composition_of(8, &mut concept_a, &mut example_concept);
